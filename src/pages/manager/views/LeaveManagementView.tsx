...
import { CheckCircle, XCircle, Clock, Users, Calendar as CalendarIcon, Mail } from 'lucide-react';
...
export const LeaveManagementView = ({ employees, user }: LeaveManagementViewProps) => {
  ...
  // Create notification for employee when leave request status changes
  const createStatusNotification = async (leaveRequest: LeaveRequest, status: 'approved' | 'rejected') => {
    try {
      const notificationData = {
        title: `Leave Request ${status.charAt(0).toUpperCase() + status.slice(1)}`,
        message: `Your leave request from ${format(parseISO(leaveRequest.startDate), 'MMM dd, yyyy')} to ${format(parseISO(leaveRequest.endDate), 'MMM dd, yyyy')} has been ${status}.`,
      ...
      await makeAuthenticatedRequest(`${API_BASE_URL}/notifications`, {
        method: 'POST',
        body: JSON.stringify(notificationData)
      });

      console.log(`✅ Notification created for employee ${leaveRequest.employeeId}`);
    } catch (error) {
      console.error('❌ Error creating notification:', error);
      // Don't throw error here - we still want to update the leave request even if notification fails
    }
  };

  const updateLeaveRequest = async (requestId: string, status: 'approved' | 'rejected', comments?: string) => {
    try {
      // Find the current request to get employee info
      const currentRequest = leaveRequests.find(req => req.id === requestId);
      if (!currentRequest) {
      ...
      const updatedRequest = await makeAuthenticatedRequest(`${API_BASE_URL}/leave-requests/${requestId}`, {
        method: 'PUT',
        body: JSON.stringify(updateData)
      });

      // Create notification for the employee
      await createStatusNotification(currentRequest, status);

      toast({
        title: "Success",
        description: `Leave request ${status}. Employee has been notified.`,
      });
  ...
  const sendReminder = async (request: LeaveRequest) => {
    try {
      const notificationData = {
        title: "Leave Request Reminder",
        message: `Your leave request from ${format(parseISO(request.startDate), 'MMM dd, yyyy')} to ${format(parseISO(request.endDate), 'MMM dd, yyyy')} is still pending review.`,
        type: 'leave_reminder',
      ...
      toast({
        title: "Reminder Sent",
        description: `Reminder sent to ${request.employeeName}`,
      });
    } catch (error) {
  ...
  // Set up auto-refresh every 30 seconds to catch real-time updates
  useEffect(() => {
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {request.employeeEmail || 'No email'}
                        </p>
                      ...
                      <div className="flex gap-2">
                        ...
                        {(request.status === 'approved' || request.status === 'rejected') && request.reviewedByName && (
                          <div className="text-xs text-muted-foreground">
                            {request.status} by {request.reviewedByName}
                            {request.reviewedAt && (
                              <span> on {format(parseISO(request.reviewedAt), 'MMM dd, yyyy')}</span>
                            )}
                          </div>
                        )}
                      </div>
